...
class AdminController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new CActiveDataProvider('Vacancies', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
				...
				if(Yii::app()->getRequest()->getParam('enablesave') == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-vacancies',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'Vacancies success created.').'</strong></div>',
						));
					} else {
				...
				if(Yii::app()->getRequest()->getParam('enablesave') == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-vacancies',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'Vacancies success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				Vacancies::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				Vacancies::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-vacancies',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'Vacancies success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'Vacancies Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
